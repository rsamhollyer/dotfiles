...
'-h[Display the concise help for this command]' \
'--help[Display the concise help for this command]' \
':service -- The domain or URL of the service to logout from:_default' \
&& ret=0
;;
(token)
_arguments "${_arguments_options[@]}" : \
...
'*--no-group=[Disable the specified dependency group \[env\: \`UV_NO_GROUP\`=\]]:NO_GROUP:' \
'(--group --dev --all-groups)*--only-group=[Only include dependencies from the specified dependency group]:ONLY_GROUP:' \
'*--no-editable-package=[Install the specified editable packages as non-editable]:NO_EDITABLE_PACKAGE:' \
...
'*--upgrade-group=[Allow upgrades for all packages in a dependency group, ignoring pinned versions in any existing output file]:UPGRADE_GROUP:_default' \
...
'*--no-sources-package=[Don'\''t use sources from the \`tool.uv.sources\` table for the specified packages \[env\: \`UV_NO_SOURCES_PACKAGE\`=\]]:NO_SOURCES_PACKAGE:_default' \
'*--no-build-package=[Don'\''t build source distributions for a specific package \[env\: \`UV_NO_BUILD_PACKAGE\`=\]]:NO_BUILD_PACKAGE:' \
'*--no-binary-package=[Don'\''t install pre-built wheels for a specific package \[env\: \`UV_NO_BINARY_PACKAGE\`=\]]:NO_BINARY_PACKAGE:' \
...
'*--no-editable-package=[Don'\''t add the specified requirements as editable]:NO_EDITABLE_PACKAGE:' \
...
'(--frozen --no-sync --only-install-project)--no-install-project[Do not install the current project \[env\: UV_NO_INSTALL_PROJECT=\]]' \
'(--frozen --no-sync --no-install-project)--only-install-project[Only install the current project]' \
'(--frozen --no-sync --only-install-workspace)--no-install-workspace[Do not install any workspace members, including the current project \[env\: UV_NO_INSTALL_WORKSPACE=\]]' \
'(--frozen --no-sync --no-install-workspace)--only-install-workspace[Only install workspace members, including the current project]' \
'(--frozen --no-sync --only-install-local)--no-install-local[Do not install local path dependencies \[env\: UV_NO_INSTALL_LOCAL=\]]' \
...
'--no-active[Prefer project'\''s virtual environment over an active environment]' \
'(--only-install-project)--no-install-project[Do not install the current project \[env\: UV_NO_INSTALL_PROJECT=\]]' \
'(--no-install-project)--only-install-project[Only install the current project]' \
'(--only-install-workspace)--no-install-workspace[Do not install any workspace members, including the root project \[env\: UV_NO_INSTALL_WORKSPACE=\]]' \
'(--no-install-workspace)--only-install-workspace[Only install workspace members, including the root project]' \
'(--only-install-local)--no-install-local[Do not install local path dependencies \[env\: UV_NO_INSTALL_LOCAL=\]]' \
...
'*--no-group=[Disable the specified dependency group \[env\: \`UV_NO_GROUP\`=\]]:NO_GROUP:_default' \
'(--group --dev --all-groups)*--only-group=[Only include dependencies from the specified dependency group]:ONLY_GROUP:_default' \
'*--no-editable-package=[Export the specified editable packages as non-editable]:NO_EDITABLE_PACKAGE:' \
...
(check)
_arguments "${_arguments_options[@]}" : \
'(--extra --all-extras --no-extra --no-all-extras --dev --no-dev --only-dev --group --no-group --no-default-groups --only-group --all-groups --no-project)--script=[Run checks for the specified PEP 723 Python script, rather than the current project]:SCRIPT:_files' \
'(--all-extras --only-group)*--extra=[Include optional dependencies from the specified extra name]:EXTRA:' \
'*--no-extra=[Exclude the specified optional dependencies, if \`--all-extras\` is supplied]:NO_EXTRA:' \
'(--only-group --only-dev)*--group=[Include dependencies from the specified dependency group]:GROUP:' \
'*--no-group=[Disable the specified dependency group \[env\: \`UV_NO_GROUP\`=\]]:NO_GROUP:' \
...
'-p+[The Python interpreter to use for the project environment]:PYTHON:' \
'--python=[The Python interpreter to use for the project environment]:PYTHON:' \
'--ty-version=[The version of ty to use for type checking]:TY_VERSION:' \
'*--index=[The URLs to use when resolving dependencies, in addition to the default index]:INDEX:_default' \
'--default-index=[The URL of the default package index (by default\: <https\://pypi.org/simple>)]:DEFAULT_INDEX:_default' \
'-i+[(Deprecated\: use \`--default-index\` instead) The URL of the Python package index (by default\: <https\://pypi.org/simple>)]:INDEX_URL:_default' \
'--index-url=[(Deprecated\: use \`--default-index\` instead) The URL of the Python package index (by default\: <https\://pypi.org/simple>)]:INDEX_URL:_default' \
...
(audit)
_arguments "${_arguments_options[@]}" : \
'*--no-extra=[Don'\''t audit the specified optional dependencies]:NO_EXTRA:' \
'*--no-group=[Don'\''t audit the specified dependency group \[env\: \`UV_NO_GROUP\`=\]]:NO_GROUP:' \
'*--only-group=[Only audit dependencies from the specified dependency group]:ONLY_GROUP:' \
'--output-format=[Select the output format]:OUTPUT_FORMAT:((text\:"Display the result in a human-readable format"
json\:"Display the result in JSON format"
...
arm64-apple-ios-simulator\:"An ARM64 target for iOS simulator"
x86_64-apple-ios-simulator\:"An \`x86_64\` target for iOS simulator"))' \
'*--ignore=[Ignore a vulnerability by ID]:IGNORE:_default' \
'*--ignore-until-fixed=[Ignore a vulnerability by ID, but only while no fix is available]:IGNORE_UNTIL_FIXED:_default' \
...
'(--no-dev)--only-dev[Only audit the development dependency group]' \
'(--frozen -U --upgrade)--locked[Assert that the \`uv.lock\` will remain unchanged \[env\: UV_LOCKED=\]]' \
...
(install)
_arguments "${_arguments_options[@]}" : \
'*-r+[Install the packages listed in the given files]:REQUIREMENTS:_files' \
...
&& ret=0
    ...
    (workspace)
        ...
        case $line[1] in
            (metadata)
_arguments "${_arguments_options[@]}" : \
'--script=[View metadata for the specified PEP 723 Python script, rather than the current workspace]:SCRIPT:_files' \
'*--index=[The URLs to use when resolving dependencies, in addition to the default index]:INDEX:_default' \
'--default-index=[The URL of the default package index (by default\: <https\://pypi.org/simple>)]:DEFAULT_INDEX:_default' \
'-i+[(Deprecated\: use \`--default-index\` instead) The URL of the Python package index (by default\: <https\://pypi.org/simple>)]:INDEX_URL:_default' \
'--index-url=[(Deprecated\: use \`--default-index\` instead) The URL of the Python package index (by default\: <https\://pypi.org/simple>)]:INDEX_URL:_default' \
...
'--project=[Discover a project in the given directory]:PROJECT:_files -/' \
'--config-file=[The path to a \`uv.toml\` file to use for configuration]:CONFIG_FILE:_files' \
'(--frozen -U --upgrade)--locked[Check if the lockfile is up-to-date \[env\: UV_LOCKED=\]]' \
'(--locked)--frozen[Assert that a \`uv.lock\` exists without checking if it is up-to-date \[env\: UV_FROZEN=\]]' \
'(--frozen --locked --sync)--dry-run[Perform a dry run, without writing the lockfile]' \
'--no-index[Ignore the registry index (e.g., PyPI), instead relying on direct URL dependencies and those provided via \`--find-links\`]' \
'-U[Allow package upgrades, ignoring pinned versions in any existing output file. Implies \`--refresh\`]' \
'--upgrade[Allow package upgrades, ignoring pinned versions in any existing output file. Implies \`--refresh\`]' \
'--no-upgrade[]' \
...
'upgrade:Upgrade a dependency in the project' \
...
_uv__cache_commands() {
    local commands; commands=(
'clean:Clear the cache, removing all entries or those linked to specific packages' \
'prune:Prune dangling cache entries and cached environments' \
...
(( $+functions[_uv__check_commands] )) ||
_uv__check_commands() {
    local commands; commands=()
    _describe -t commands 'uv check commands' commands "$@"
}
...
(( $+functions[_uv__upgrade_commands] )) ||
_uv__upgrade_commands() {
    local commands; commands=()
    _describe -t commands 'uv upgrade commands' commands "$@"
}
